...
- **Algorithm Fidelity**: No change to FSRS v6 scheduling math or card-state transition rules. User-initiated deck deletion removes related learning-state records by design, but does not alter scheduling logic for remaining records.
...
- **FR-014**: System MUST preserve existing FSRS v6 scheduling fidelity, card-state transitions, and account-level daily learning limits without introducing behavioral changes.
