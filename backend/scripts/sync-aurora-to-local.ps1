...
Write-Host "Source (aurora): $($aurora.Host):$($aurora.Port)/$($aurora.DbName) via instance $instanceId" -ForegroundColor Cyan
Write-Host "Target (local): $localHost`:$localPort/$localDbName" -ForegroundColor Yellow

if (-not $Force) {
  $answer = Read-Host 'This will drop and recreate local DB before import. Type YES to continue'
  if ($answer -ne 'YES') {
...
$dumpFile = Join-Path $dumpDirectory "aurora-to-local-$timestamp.sql"
...
try {
  ...
  Write-Host "Dumping Aurora DB to $dumpFile" -ForegroundColor Cyan
  & mysqldump -h 127.0.0.1 -P $forwardPort -u $($aurora.Username) "--password=$($aurora.Password)" --single-transaction --routines --triggers $($aurora.DbName) > $dumpFile

  Write-Host 'Resetting local target DB...' -ForegroundColor Cyan
  $resetSql = "DROP DATABASE IF EXISTS ``$localDbName``; CREATE DATABASE ``$localDbName``;"
  & mysql -h $localHost -P $localPort -u $localUser "--password=$localPass" -e $resetSql

  Write-Host 'Importing dump into local DB...' -ForegroundColor Cyan
  Get-Content -Raw $dumpFile | & mysql -h $localHost -P $localPort -u $localUser "--password=$localPass" $localDbName

  Write-Host 'Sync Aurora -> local completed.' -ForegroundColor Green
} finally {
